...
p = input("Enter pipe color [red or green] : ")
if p == 'red':
    pc = pygame.image.load('pipe-red.png')
if p == 'green':
    pc = pygame.image.load('pipe-green.png')

game_speed = input("Enter the game difficulty [1 is the highest]: ")
...
# bird_surface = pygame.image.load('bluebird-midflap.png').convert()
...
pygame.time.set_timer(SPAWNPIPE, int(game_speed) * 1000)
...
hit = pygame.mixer.Sound('sfx_die.wav')
swoosh = pygame.mixer.Sound('sfx_swooshing.wav')
score_sound_countdown = 100

while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            Flappy_Score_file = open("Flappy_Score", "w")
            Flappy_Score_file.write(str(int(high_score)))
            Flappy_Score_file.close()
            pygame.quit()
            sys.exit()
